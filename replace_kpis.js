...
let content = fs.readFileSync('src/views/ReportesBI.js', 'utf8');

const startStr = '<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "16px", marginBottom: "24px" }}>';
const endStr = '{/* KPI 9: Producción de Leche (Promedio Individual) */}';

const startIndex = content.indexOf(startStr);
const tempIndex = content.indexOf(endStr, startIndex);
const endIndex = content.indexOf('</div>', tempIndex + 100) + 6;

if (startIndex !== -1 && endIndex !== -1) {
    const newGrid = \<div className="kpi-grid" style={{ marginBottom: "24px" }}>
    ...
    fs.writeFileSync('src/views/ReportesBI.js', content);
    console.log("Success");
} else {
